...
$user_id = $_SESSION['id'];
$subject = $_GET['subject'] ?? 'Business Studies';

// AJAX Save-Note Handler
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $title = htmlspecialchars($_POST['title'] ?? '');
    $content = htmlspecialchars($_POST['content'] ?? '');

    if ($title && $content) {
        $stmt = $conn->prepare("INSERT INTO notes (user_id, title, content, subject_name) VALUES (?, ?, ?, ?)");
...
<body>
    ...
    <div id="notes-list">
        ...
        <?php
        $load = $conn->prepare("SELECT title, content, created_at FROM notes WHERE user_id = ? AND subject_name = ? ORDER BY created_at DESC");
    ...
    function saveNote() {
        ...
        if (!title || !content) {
            messageDiv.textContent = "Please enter title and content.";
            return;
        }
